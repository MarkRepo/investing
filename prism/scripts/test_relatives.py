...
def test_set_parent_materials_resolves_omitted_variant_confident():
    """省略 parent_variant 且父唯一变体 → resolve confident 落盘 chosen。"""
    with _sandbox():
...
def test_set_parent_materials_raises_when_not_confident():
    """省略 parent_variant 且父多个异模型+含未登记 → raise 列候选（不瞎猜）。"""
    import pytest
    with _sandbox():
        topic_io.create_topic("ind", "行业", "industry", "Q", "CN", "deep",
                              "claude-opus-4-7")
        # 第二个父变体：未登记模型（create_topic passthrough，dir 名原样）
        topic_io.create_topic("ind", "行业", "industry", "Q", "CN", "deep",
                              "foo-unknown")
        topic_io.create_topic("co", "公司", "company", "Q", "CN", "deep",
                              "gpt-5-4", ticker="SZSE_002460", short_name="赣锋")
        with pytest.raises(ValueError):
            topic_io.set_parent_materials("co", "gpt-5-4", [
                {"parent_slug": "ind", "mat_id": "mat-x"},   # 省略 → 拿不准 → raise
            ])
    print("✓ test_set_parent_materials_raises_when_not_confident")
...
if __name__ == "__main__":
    ...
    test_set_parent_materials_resolves_omitted_variant_confident()
    test_set_parent_materials_raises_when_not_confident()
    test_findings_read_bridges_naming_split()
