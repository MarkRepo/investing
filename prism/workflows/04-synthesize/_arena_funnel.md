...
**thesis_v1（决策链跑完后才写）**：照 `_shared.md` § thesis_v1 的 **Scheme C 全快照 11 段式**，不改。

**收尾**：照 `_shared.md` § 全部产出完成后——`append_user_todos` + 清 `next_actions` + stage 推进。arena 完成后 stage 置 `10-peer-matrix` 或 `done`（与旧 10 Step 8 一致）；critic 对 arena 非强制（可选）。
