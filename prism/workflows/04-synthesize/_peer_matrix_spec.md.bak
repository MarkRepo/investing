...
写入文件：`prism/topics/{slug}/{variant}/outputs/peer_matrix.yaml`
...
chain_links:                          # 决策链 6 环结构断言（被动断链探针 04.Q1 读）
  ...
  r4_anchors_r2: {true|false}         # ④横比矩阵是否锚回②的隐含数
...
{上面填好的 yaml 内容}
'''
path = Path('prism/topics/{slug}/{variant}/outputs/peer_matrix.yaml')
