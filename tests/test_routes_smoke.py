...
def test_industries_observations_route_removed(client, tmp_path):
    """Task 8: /industries/{slug}/observations route is removed; returns 404."""
    from app.io import industry as industry_io
    industry_io.create_industry(
...
def test_industries_detail_no_observations_link(client, tmp_path):
    """Task 8: detail page no longer links to the removed /observations route."""
    from app.io import industry as industry_io
    industry_io.create_industry(
    ...
    assert r.status_code == 200
    assert "/industries/cn-linked/observations" not in r.text
